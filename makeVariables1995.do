...
//\item|x4111| is how many weeks per year R works;
//\item |x4110| is how many hours per week R works.
//\end{itemize}
// We use weeks-per-year to measure total labor income when the given payment frequency is anything but annual, and we use hours per week to measure it when it is hourly.
gen resp = 0
...
//\end{itemize}
//We apply them to the measurement of the spouse's labor income analogously
gen spouse = 0
...
//Many of the total contributions by R exceed the IRS Section 402(g) limit of 9240 for tax year 1994. We replace these obvious overestimations with this statuatory limit.
replace employersponsoredR=9240 if employersponsoredR>9240
...
replace employersponsoredS=9240 if employersponsoredS>9240
...
// For this, we first measure the household's AGI and then apply the relevant tax table from 1994.
/* The variables used for this measurement are
\begin{itemize}
\item x101, number of people living in the household/primary economic unit.
\item x5744, indicator for whether the respondent's household has or expects to file a year 1994 Federal Income Tax Return (1=YES - FILED; 5=NO - DO NOT EXPECT TO FILE; 6=YES - NOT YET FILED)
...
//
// These calculations assume that respondents with a spouse that said only R filed (|x5746==3| and other data indicate presence of a spouse)or respondents indicating that only S filed (|x5746==4|) actually are married filing jointly.
gen spousePresent= (x8020==2 | x8020==3| x102==2 | x102==3 | x100==1)

gen mfj = (x5744==1 | x5744==6) & (x5746==1 | (x5746==3 & spousePresent==1) | x5746==4) // Indicator for married filing jointly.
...
// Read the tax paid off of the 1994 tax rate schedules on page 53 of Publication i1040.
replace tax = agi*0.15  					if agi<=38000 					& mfj==1
replace tax = 5700.00+0.28*(agi-38000) 		if agi>38000 & agi<=91850 		& mfj==1 
replace tax = 20778.00+0.31*(agi-91850) 	if agi>91850 & agi<=140000		& mfj==1
replace tax = 35704.50+0.36*(agi-140000) 	if agi>140000 & agi<=250000 	& mfj==1
replace tax = 75304.50+0.396*(agi-250000) 	if agi>250000 					& mfj==1
...
//Read the respondent's taxes off of the 1994 tax table.
replace taxR=agiR*0.15 						if agiR<= 19000 				& mfs==1
replace taxR=2850.00+0.28*(agiR-19000) 		if agiR>19000 & agiR<=44925 	& mfs==1
replace taxR=10389.00+0.31*(agiR-44925) 	if agiR>44925 & agiR<=70000 	& mfs==1
replace taxR=17852.25+0.36*(agiR-70000) 	if agiR>70000 & agiR<=125000 	& mfs==1
replace taxR=37652.25+0.396*(agiR-125000) 	if agiR>125000					& mfs==1

//Read the spouse's taxes off of the 1994 tax table.
replace taxS=agiS*0.15 						if agiS<= 19000 				& mfs==1
replace taxS=2850.00+0.28*(agiS-19000) 		if agiS>19000 & agiS<=44925 	& mfs==1
replace taxS=10389.00+0.31*(agiS-44925) 	if agiS>44925 & agiS<=70000 	& mfs==1
replace taxS=17852.25+0.36*(agiS-70000) 	if agiS>70000 & agiS<=125000 	& mfs==1
replace taxS=37652.25+0.396*(agiS-125000) 	if agiS>125000					& mfs==1
...
gen single=(x5744==1 | x5744==6) & x5746==3 & spousePresent==0 & x101==1

gen agi=x7651 if single==1 //respondent's AGI
replace agi=0 if agi<0

replace tax = agi*0.15 						if agi<=22750 					& single==1
replace tax = 3412.50+0.28*(agi-22750) 		if agi>22750 & agi<=55100 		& single==1
replace tax = 12470.50+0.31*(agi-55100) 	if agi>55100 & agi<=115000 		& single==1
replace tax = 31039.50+0.36*(agi-115000) 	if agi>115000 & agi<=250000 	& single==1
replace tax = 79639.50+0.396*(agi-250000) 	if agi>250000 					& single==1

drop agi

//\subsection{Heads of Household}
gen hh=(x5744==1 | x5744==6) & x5746==3 & spousePresent==0 & x101>1
gen agi=x7651 if hh==1 //respondent's AGI
replace agi=0 if agi<0

replace tax=agi*0.15 						if agi<=30050 					& hh==1
replace tax=4575.00+0.28*(agi-30050) 		if agi>33050 & agi<=78700 		& hh==1
replace tax=18071.00+0.31*(agi-78700) 		if agi>78700 & agi<=127500 		& hh==1
replace tax=33199.00+0.36*(agi-127500) 		if agi>127500 & agi<=250000 	& hh==1
replace tax=77299.00+0.396*(agi-250000) 	if agi>250000 					& hh==1
...
//\begin{itemize}
//\item FICA taxes ($6.2$ pecent on all wages and salaries at or under \$60,600 in 1994, see page 2 of Publication 15),
//\item Self-employment tax (15.3 percent on all business income, see lines 10 and 11 of Part I of Form 1040 Schedule SE),
//\item Medicare taxes ($1.45$ percent on all wages and salaries, see page 2 of Publication 15), 
...
// Fica tax, 6.2% on amounts at or under \$60,600
gen ficaResp=0.062*respShare*x5702 
gen ficaSpouse=0.062*spouseShare*x5702 

//If either partner's calculated tax would exceed $0.062\times 60,600=3757.20$ dollars, then truncate the tax payments at this amount.
replace ficaResp	=3757.20 	if ficaResp>3757.20
replace ficaSpouse	=3757.20 	if ficaSpouse>3757.20
...
replace fica=3757.20 if respShare==. & fica>3757.20
...
// We estimate IRA contributions by assuming that each partner makes the maximum (possible) deductable contribution (\$2,000 in year 1994, see Page 7 of Publication 590) if 
//\begin{itemize}
...
gen year = 1995 //|bulletin.do| requires this variable, which gives the year of the \emph{SCF survey}, not the tax year.
quietly: do bulletin
