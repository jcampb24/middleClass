...
//Many of the total contributions by R exceed the IRS Section 402(g) limit of 9500 for tax year 1997. We replace these obvious overestimations with this statuatory limit.
replace employersponsoredR=9500 if employersponsoredR>9500
...
replace employersponsoredS=9500 if employersponsoredS>9500
...
// Read the tax paid off of the 1997 tax rate schedules on page 51 on Form i1040.
replace tax = agi*0.15 if agi<=41200 & mfj==1
replace tax = 6180.00+0.28*(agi-41200) if agi>41200 & agi<=99600 & mfj==1 
replace tax = 22532.00+0.31*(agi-99600) if agi>99600 & agi<=151750 & mfj==1
replace tax = 41170.50+0.36*(agi-151750) if agi>151750 & agi<=271050 & mfj==1
replace tax = 86854.50+0.396*(agi-271050) if agi>271050 & mfj==1
...
//Read the respondent's taxes off of the 1997 tax table.
replace taxR=agiR*0.15 if agiR<= 20600 & mfs==1
replace taxR=3090.00+0.28*(agiR-20600) if agiR>20600 & agiR<=49800 & mfs==1
replace taxR=11266.00+0.31*(agiR-49800) if agiR>49800 & agiR<=75875 & mfs==1
...
replace taxS=19345.25+0.36*(agiS-75875) if agiS>75875 & agiS<=135525 & mfs==1
replace taxS=40823.25+0.396*(agiS-135525) if agiS>135525 & mfs==1
...
replace tax = agi*0.15 if agi<=24650 & single==1
replace tax = 3697.50+0.28*(agi-24650) if agi>24650 & agi<=59750 & single==1
replace tax = 13525.50+0.31*(agi-59750) if agi>59750 & agi<=124650 & single==1
replace tax = 33644.50+0.36*(agi-124650) if agi>124650 & agi<=271050 & single==1
replace tax = 86348.50+0.396*(agi-271050) if agi>271050 & single==1
...
replace tax=agi*0.15 if agi<=33050 & hh==1
replace tax=4957.50+0.28*(agi-33050) if agi>33050 & agi<=85350 & hh==1
replace tax=19601.50+0.31*(agi-85350) if agi>85350 & agi<=138200 & hh==1
replace tax=35985.00+0.36*(agi-138200) if agi>138200 & agi<=271050 & hh==1
replace tax=83811.00+0.396*(agi-271050) if agi>271050 & hh==1
...
//\begin{itemize}
//\item FICA taxes ($6.2$ pecent on all wages and salaries at or under \$65,400 in 1997, see page 2 of Publication 15),
//\item Self-employment tax (15.3 percent on all business income, see page 11 of Publication 533),
...
// Fica tax, 6.2% on amounts at or under \$65,400
gen ficaResp=0.062*respShare*x5702 
gen ficaSpouse=0.062*spouseShare*x5702 

//If either partner's calculated tax would exceed $0.062\times 65400=4054.80$ dollars, truncate it at this amount.
replace ficaResp=4054.80 if ficaResp>4054.80
replace ficaSpouse=4054.80 if ficaSpouse>4054.80
...
replace fica=4054.80 if respShare==. & fica>4054.80
...
// We estimate IRA contributions by assuming that each partner makes the maximum (possible) deductable contribution (\$2,000 in year 1997, see Page 7 of Publication 590) if 
//\begin{itemize}
...
gen year = 1998 //|bulletin.do| requires this variable, which gives the year of the \emph{SCF survey}, not the tax year.
quietly: do bulletin
