//makeVariables2004.do
//This program loads the 2004 Survey of Consumer Finance data and generates variables of interest for later analysis.
...
// The variables used to calculate the household's year 2003 contributions to employer-sponsored plans are
/* \begin{itemize}
...
//Many of the total contributions by R exceed the IRS Section 402(g) limit of \$12,000 for tax year 2003. We replace these obvious overestimations with this statuatory limit.
replace employersponsoredR=12000 if employersponsoredR>12000
...
replace employersponsoredS=12000 if employersponsoredS>12000 
...
// Read the tax paid off of the 2003 tax table.
replace tax = agi*0.10 						if agi<=14000 				& mfj==1
replace tax = 1400.00+0.15*(agi-14000)		if agi>14000 & agi<=56800 	& mfj==1
replace tax = 7820.00+0.25*(agi-56800) 		if agi>56800 & agi<=114650 	& mfj==1
replace tax = 22282.50+0.28*(agi-114650) 	if agi>114650 & agi<=174700 & mfj==1
replace tax = 39096.50+0.33*(agi-174700)	if agi>174700 & agi<=311950 & mfj==1
replace tax = 84389.00+0.35*(agi-311950)	if agi>311950 				& mfj==1
...
//Read the respondent's taxes off of the 2003 tax table.
replace taxR=agiR*0.10 						if agiR<=7000					& mfs==1
replace taxR=700.00+0.15*(agiR-7000)		if agiR>7000 & agiR<=28400		& mfs==1
replace taxR=3910.00+0.25*(agiR-28400)		if agiR>28400 & agiR<=57325 	& mfs==1
replace taxR=11141.25+0.28*(agiR-57325)		if agiR>57325 & agiR<=87350		& mfs==1
replace taxR=19548.25+0.33*(agiR-87350)		if agiR>87350 & agiR<=155975	& mfs==1
replace taxR=42194.50+0.35*(agiR-155975)	if agiR>155975					& mfs==1


// Read the spouse's taxes off of the 2003 tax table.
replace taxS=agiS*0.10 						if agiS<=7000					& mfs==1
replace taxS=700.00+0.15*(agiS-7000)		if agiS>7000 & agiS<=28400		& mfs==1
replace taxS=3910.00+0.25*(agiS-28400)		if agiS>28400 & agiS<=57325 	& mfs==1
replace taxS=11141.25+0.28*(agiS-57325)		if agiS>57325 & agiS<=87350		& mfs==1
replace taxS=19548.25+0.33*(agiS-87350)		if agiS>87350 & agiS<=155975	& mfs==1
replace taxS=42194.50+0.35*(agiS-155975)	if agiS>155975					& mfs==1
...
replace tax = agi*0.10 						if agi<=7000					& single==1
replace tax = 700.00+0.15*(agi-7000)		if agi>7000 & agi<=28400		& single==1
replace tax = 3910.00+0.25*(agi-28400)		if agi>28400 & agi<=68800		& single==1
replace tax = 14010.00+0.28*(agi-68800)		if agi>68800 & agi<=143500		& single==1
replace tax = 34926.00+0.33*(agi-143500)	if agi>143500 & agi<=311950		& single==1
replace tax = 90514.50+0.35*(agi-311950)	if agi>311950					& single==1
...
replace tax = 0.10*agi						if agi<=10000					& hh==1
replace tax = 1000.00+0.15*(agi-10000)		if agi>10000 & agi<=38050		& hh==1
replace tax = 5207.50+0.25*(agi-38050)  	if agi>38050 & agi<=98250		& hh==1
replace tax = 20257.50+0.28*(agi-98250)		if agi>98250 & agi<=159100		& hh==1
replace tax = 37295.50+0.33*(agi-159100)	if agi>159100 & agi<=311950		& hh==1
replace tax = 87736.00+0.35*(agi-311950)	if agi>311950					& hh==1
...
//\begin{itemize}
//\item FICA taxes ($6.2$ pecent on all wages and salaries at or under \$87,000 in 2003, see page 1 of Publication 15),
//\item Self-employment tax (15.3 percent on all business income, see page 2 of Publication 533),
//\item Medicare taxes ($1.45$ percent on all wages and salaries, see page 1 of Publication 15), 
...
//If either partner's calculated tax would exceed $0.062\times 87000=5394$ dollars, truncate it at this amount.
replace ficaResp=5394.00 if ficaResp>5394.00
replace ficaSpouse=5394.00 if ficaSpouse>5394.00
...
replace fica=5394.00 if respShare==. & fica>5394.00
...
// We estimate IRA contributions by assuming that each partner makes the maximum (possible) deductable contribution (\$3,000 in year 2003 for individuals under 50, see Publication 590) if 
//\begin{itemize}
...
gen ira=(x4137~=1 & x4737~=1)*(3000*(x3602==1)+3000*(x3612==1))
...
gen year = 2004 //|bulletin.do| requires this variable.
