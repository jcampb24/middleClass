//sampleSizeTable.do
//This program creates the table that displays the number of U.S. households represented in the initial SCF sample and the number remaining after each of the applied sample selection criteria.

set more off
set maxvar 10000

//Create a temporary file used to store the table's entries as we accumulate them across waves of the SCF.
...
keep nhouse*
keep if _n==1
order nhouseholdsStart nhouseholdsNotImputed nhouseholdsAge nhouseholdsNoTANF nhouseholdsNonPoverty nhouseholdsWealth nhouseholdsSelfEmployed 
xpose, clear
replace v1=v1/1000000 //Express the number of sample households in millions.
format v1 %3.1f

gen labels=""
replace labels="Households$^{\textrm{(i)}}$" if _n==1
...
listtab using scfSample.tex, delimiter("&") end("\\") replace
